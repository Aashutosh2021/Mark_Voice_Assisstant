...
DB_PATH = "mark_memory/chat_history.db"
...
@function_tool()
async def control_microphone(prompt: str, action: Literal["mute", "unmute", "toggle"] = "mute") -> str:
    """Mutes, unmutes, or toggles the default microphone."""
    try:
        device = AudioUtilities.GetMicrophone()
        if device is None:
            return "❌ No active microphone device was found."

        interface = device.Activate(IAudioEndpointVolume._iid_, CLSCTX_ALL, None)
        mic_volume = cast(interface, POINTER(IAudioEndpointVolume))
        current_state = mic_volume.GetMute()
        ...
        return f"🎙️ Microphone {status}."
    except Exception as e:
        logger.error(f"Microphone control failed: {e}")
        return f"❌ Failed to control microphone: {str(e)}"
    ...
    <body>
        <div class="container">
            ...
            <div class="footer">
                ...
                <p>Advanced Groundwater Analysis Engine v2.0 • Powered by Mark</p>
            </div>
        </div>
...
Return ONLY the code without any explanations or markdown formatting."""
        ...
        # ✅ FIXED: Save file directly without typing
        logger.info(f"💾 Saving well-formatted {lang} code to file...")
        
        # Save directly to file
        try:
            # Use current directory or a safe location
            save_path = os.path.join(os.getcwd(), full_filename)
            ...
            try:
                ...
                logger.info("📝 Opened file in default editor")
            except Exception as open_error:
                logger.warning(f"Could not auto-open file: {open_error}")
