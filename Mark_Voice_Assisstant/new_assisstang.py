...
class Assistant(Agent):
    ...
    def _build_instructions(self) -> str:
        ...
        return "\n".join([
            ...
            "You have access to the user's camera feed. You can see the user and their environment. If the user shows you something, analyze it visually.", # <--- ADD THIS LINE
            f"\nAvailable tools:\n{available_tools}",
...
async def entrypoint(ctx: agents.JobContext):
    ...
    # Import RoomOptions to fix the deprecation warning
    from livekit.agents import RoomOptions  

    for attempt in range(1, max_retries + 1):
        try:
            print(f"🔄 Connecting to room... (Attempt {attempt})")
            
            # Create agent and session
            agent = Assistant()
            session = AgentSession()
            
            # Start session with NEW RoomOptions syntax
            # We enable video=True so it can see what you publish
            # We disable audio=True (for the agent's input) so it doesn't grab YOUR mic locally
            await asyncio.wait_for(
                session.start(
                    room=ctx.room,
                    agent=agent,
                    options=RoomOptions(
                        video_enabled=True,  # Agent can SEE
                        audio_enabled=True,  # Agent can HEAR
                    ),
                ),
                timeout=20.0
            )
            ...
            if os.getenv("NOVA_VARIANT", "base").lower() in ["elite", "premium"]:
