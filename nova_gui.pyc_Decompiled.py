...
class NOVA_AI(QWidget):
    ...
    def update_network_info(self):
        ...
        self.network_status_widget.layout().itemAt(1).widget().setText(f'WiFi (NOVA_5G) - {signal_strength}')
...
class SystemStatsPanel(QWidget):
    ...
    def update_mock_data(self):
        """Update with mock data for demonstration"""  # inserted
        ...
        if np.random.random() < 0.2:
            ...
            status, temp = weather_options[np.random.randint(0, len(weather_options))]
            self.weather_status = status
            self.weather_temp = temp
            self.weather_icon.set_weather_type(status)
        self.update_widgets()
...
class LinearProgressBar(QWidget):
    ...
    def paintEvent(self, event):
        ...
        progress_ratio = max(0.0, min(100.0, float(self.animation_value))) / 100.0
...
class TemperatureBar(LinearProgressBar):
    ...
    def paintEvent(self, event):
        ...
        painter.setBrush(QBrush(QColor(0, 60, 80, 150)))
        painter.drawRoundedRect(bg_rect, 2, 2)
        clamped_value = max(0.0, min(100.0, float(self.animation_value)))
        progress_width = int(self.width() * (clamped_value / 100.0))
        progress_rect = QRectF(0, 4, progress_width, 4)
        gradient = QLinearGradient(0, 0, progress_width, 0)
        ...
        if self.weather_type == 'CLEAR':
            ...
            if self.weather_type == 'CLOUDY':
                ...
                if self.weather_type == 'RAIN':
                    ...
                    for i in range(3):
                        painter.drawLine(15, i + 8, 22, 13 + i + 8)
                        painter.drawLine(25, i + 8, 32, 13 + i + 8)
                else:  # inserted
...
class NOVAInterfaceWidget(QWidget):
    ...
    def init_nova_interface(self):
        """Initialize the original NOVA interface"""  # inserted
    ...
    def draw_center_text(self, painter: QPainter, center: QPointF, pulse: float):
        """Draw NOVA text with neon effects"""  # inserted
...
class NOVAInterfaceWindow(QMainWindow):
    ...
    def set_assistant(self, assistant):
        ...
        if hasattr(self.nova_widget, 'right_panel') and self.nova_widget.right_panel:
            self.nova_widget.right_panel.assistant = assistant
            print(f'✅ Right panel assistant set: {assistant is not None}')
            if hasattr(self.nova_widget.right_panel, 'camera_available'):
                print(f'✅ Camera available: {self.nova_widget.right_panel.camera_available}')
                if self.nova_widget.right_panel.camera_available:
    ...
    def test_camera_frame(self):
        ...
        if hasattr(self.nova_widget, 'right_panel') and self.nova_widget.right_panel and self.nova_widget.right_panel.assistant:
            print('🧪 Testing camera frame sending...')
            self.nova_widget.right_panel.update_camera_feed()

    def trigger_special_effect(self):
        """Thread-safe effect trigger"""  # inserted
        QTimer.singleShot(0, self.nova_widget.trigger_special_effect)
    ...
    def setup_tts_callbacks(self):
        ...
        if hasattr(self, 'tts'):
            self.tts._on_speech_start = self.nova_widget.trigger_special_effect
            self.tts._on_speech_end = self.on_speech_end
        self.speech_effect_callback = self.nova_widget.trigger_special_effect
...
def init_firebase_from_embedded(database_url=None):
    ...
    firebase_admin.initialize_app(cred, {'databaseURL': database_url or 'https://novavoiceassitant-default-rtdb.firebaseio.com'})
...
def process_upgrade(current_variant):
    ...
    set_env_variable('NOVA_VARIANT', next_variant)
    ...
    os.environ['NOVA_VARIANT'] = next_variant
...
def activation_gate():
    ...
    os.environ['IS_ACTIVATED'] = 'true'
    os.environ['ACTIVATION_COUNT'] = '1'
    os.environ['NOVA_VARIANT'] = variant
...
class WaitForInternetDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle('NOVA - Waiting for Internet')
        self.setFixedSize(400, 150)
        self.setWindowFlags(self.windowFlags() & ~Qt.WindowContextHelpButtonHint)
        layout = QVBoxLayout()
        self.message_label = QLabel('🔍 Checking internet connection...\nNOVA requires internet to start')
...
class CountdownDialog(QDialog):
    ...
    def __init__(self, message, countdown_time=3, parent=None):
        ...
        self.setWindowTitle('NOVA AI - Face Authentication')
...
class SuccessDialog(QDialog):
    ...
    def __init__(self, message, parent=None):
        super().__init__(parent)
        self.setWindowTitle('NOVA AI - Success')
...
class ErrorDialog(QDialog):
    ...
    def __init__(self, message, parent=None):
        super().__init__(parent)
        self.setWindowTitle('NOVA AI - Error')
        self.setFixedSize(350, 180)
        self.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint)
...
class CameraNotAvailableDialog(QDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle('NOVA AI - Camera Not Available')
    ...
    try:
        ...
        while time.time() < start_time < CAMERA_TIMEOUT:
            ...
            if face_detected or frame_count + 10 == 0:
                ...
                cv2.imshow('NOVA AI - Face Enrollment', display_frame)
        ...
        if captured:
            if custom_model_created:
                ...
                show_success_dialog('✅ Face Enrollment Successful!\n\nYour face has been registered with NOVA AI.')
    ...
    try:
        while time.time() < verification_start < CAMERA_TIMEOUT:
            ...
            if face_detected or current_time in last_display_update > 0.1:
                ...
                cv2.putText(display_frame, 'NOVA AI - Authentication', (80, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                if custom_model_created:
                ...
                if face_detected and boxes is not None:
                    ...
                    if current_embedding is not None:
                        ...
                        if custom_model_created and distance < 0.5 or (not custom_model_created and distance < FACE_MATCH_THRESHOLD):
                            ...
                            verified = True
                cv2.imshow('NOVA AI - Face Verification', display_frame)
...
def start_nova_interface():
    """Start the actual NOVA AI interface"""  # inserted
    if not NOVA_AVAILABLE:
        show_error_dialog('❌ System Error\n\nNOVA interface components not available.')
        return False
...
if __name__ == '__main__':
    try:
        ...
        if not camera_available:
            set_env_variable(CAMERA_AVAILABLE_ENV, 'false')
            show_success_dialog('📷 Camera Not Detected\n\nStarting NOVA AI without face authentication...')
            success = start_nova_interface()
            if not success:
                sys.exit(1)
            sys.exit(0)
